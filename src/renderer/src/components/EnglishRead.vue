<template>
  <div class="english-read-container">
    ...
    <div class="left-section">
          ...
          <div class="question-text">
            ...
            >
              <template v-for="(sentence, sIndex) in paragraph.sentences" :key="`s-${pIndex}-${sIndex}`">
                ...
                <el-icon 
                  style="margin:0 12px 12px 12px; cursor: pointer; font-size: 20px;"
                  @click="copySentence(sentence.text, sentence.globalIndex)"
                >
                  <Check v-if="copiedSentences.has(sentence.globalIndex)" />
                  <CopyDocument v-else />
                </el-icon>
              </template>
            </div>
      ...
      <!-- 选项区域 -->
      <div class="options-section" v-if="displayQuestionList && displayQuestionList.length > 0" :style="{ flex: optionsFlex }">
        ...
        <div class="options-content">
          ...
          <div class="questions-container">
            <!-- 只显示当前题目 -->
            <div class="question-block" v-if="currentDisplayQuestion">
              <div class="question-number">
                <div class="question-title">
                  <div>{{ currentOptionIndex + 1 }}</div>、
                  <div 
                    v-for="(word, wordIndex) in splitIntoWords(currentDisplayQuestion.questionTitle)"
                    :key="`title-${currentOptionIndex}-${wordIndex}`"
                    class="option-word"
                  ...
                  <el-icon 
                    ...
                    @click="copyQuestionWithContext(currentOptionIndex)"
                  >
                    <Check v-if="copiedQuestions.has(currentOptionIndex)" />
                    <CopyDocument v-else />
                  </el-icon>
                </div>
              </div>
              ...
              <div class="options-list">
                <div
                  class="option-item"
                  v-for="(option, key) in getOptionsForQuestion(currentDisplayQuestion)"
                  :key="`${currentOptionIndex}-${key}`"
                  :class="{ 'deleted-option': isOptionDeleted(currentDisplayQuestion, key) }"
                >
                  <div class="option-delete">
                    <el-icon 
                      @click="toggleDeleteOption(currentDisplayQuestion, key)" 
                      :style="{ 
                        cursor: 'pointer', 
                        marginRight: '10px',
                        color: isOptionDeleted(currentDisplayQuestion, key) ? '#f56c6c' : '#909399',
                        fontSize: '20px',
                        flexShrink: 0
                      }"
                    >
                  ...
                  <div class="option-text" :style="{ textDecoration: isOptionDeleted(currentDisplayQuestion, key) ? 'line-through' : 'none' }">
                    <span 
                      v-for="(word, wordIndex) in splitIntoWords(option)"
                      :key="`${currentOptionIndex}-${key}-${wordIndex}`"
                      class="option-word"
                      @dblclick="fillSearchWord(word)"
                      @click.stop="showTranslatePopup($event, word)"
                    >
              ...
              <div class="option-buttons" style="margin: 15px 0; display: flex; gap: 10px; align-items: center;">
                <el-button 
                  v-for="(option, key) in getOptionsForQuestion(currentDisplayQuestion)" 
                  :key="key"
                  v-show="!isOptionDeleted(currentDisplayQuestion, key)"
                  :type="isOptionSelectedByRepeatId(currentDisplayQuestion.repeatId, key) ? 'primary' : 'default'" 
                  size="large" 
                  @click="selectOptionByRepeatId(currentDisplayQuestion, key)"
                ...
                <div class="answer-status">
                  <el-icon v-if="getAnswerResultByRepeatId(currentDisplayQuestion.repeatId) === 'true'" style="color: #67c23a; font-size: 25px;">
                    <Check />
                  </el-icon>
                  <el-icon v-if="getAnswerResultByRepeatId(currentDisplayQuestion.repeatId) === 'false'" style="color: #f56c6c; font-size: 25px;">
                    <Close />
                  </el-icon>
                </div>
              </div>
              ...
              <div class="explainContent">
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                  <el-button 
                    type="primary" 
                    size="large"
                    @click="toggleExplainByRepeatId(currentDisplayQuestion.repeatId)"
                  >
                    {{ showExplainByRepeatId[currentDisplayQuestion.repeatId] ? '隐藏答案' : '查看答案' }}
                  </el-button>

                  <el-button 
                    type="danger" 
                    size="large"
                    @click="hideCurrentQuestion"
                  >
                    删除
                  </el-button>
                  <div style="font-size: 16px; font-weight: bold;">
                    第 {{ currentOptionIndex + 1 }} 题 / 共 {{ displayQuestionList.length }} 题
                  </div>
                  <el-button type="primary" size="large" @click="prevOptionQuestion" :disabled="currentOptionIndex <= 0">
                    <el-icon><ArrowLeft /></el-icon> 上一题
                  </el-button>
                  <el-button type="primary" size="large" @click="nextOptionQuestion" :disabled="currentOptionIndex >= displayQuestionList.length - 1">
                    下一题 <el-icon><ArrowRight /></el-icon>
                  </el-button>
                </div>
                <div v-if="showExplainByRepeatId[currentDisplayQuestion.repeatId]" class="explain-text">
                  {{ currentDisplayQuestion.explain || '暂无解析' }}
                </div>
              </div>
            </div>
          </div>
...
import { Delete, Check, Close, ArrowLeft, ArrowRight, Refresh, Plus, List, CopyDocument } from '@element-plus/icons-vue';
...
export default {
  ...
  data() {
    return {
      ...
      copiedQuestions: new Set(), // 新增：存储已复制的题目索引
      // 选项选中状态
      ...
      // 选项题目显示相关
      displayQuestionList: [], // 复制5次后的显示数组
      currentOptionIndex: 0, // 当前显示的选项题目索引
      selectedOptionsByRepeatId: {}, // 按repeatId存储选中状态
      answerResultsByRepeatId: {}, // 按repeatId存储答题结果
      showExplainByRepeatId: {}, // 按repeatId存储解析显示状态
    };
  },
  ...
  computed: {
    ...
    currentDisplayQuestion() {
      return this.displayQuestionList[this.currentOptionIndex] || null;
    },
  ...
  methods: {
      ...
      try {
        ...
        if (response.data.code === 200 && response.data.data) {
          ...
          if (questionData.questions && questionData.questions.length > 0) {
            ...
            // 生成复制5次的显示数组
            this.generateDisplayQuestionList();
          }
        }
      } catch (error) {
    ...
    // 生成复制5次的显示数组
    generateDisplayQuestionList() {
      const questions = this.currentQuestionOptions;
      ...
      // 复制5次
      for (let i = 0; i < 5; i++) {
        repeatedList.push(...questions.map(item => ({
          ...item,
          repeatId: repeatId++
        })));
      }
      ...
      this.displayQuestionList = repeatedList;
      this.currentOptionIndex = 0;
      // 重置状态
      this.selectedOptionsByRepeatId = {};
      this.answerResultsByRepeatId = {};
      this.showExplainByRepeatId = {};
    },
    ...
    // 下一题
    nextOptionQuestion() {
      if (this.currentOptionIndex < this.displayQuestionList.length - 1) {
        this.currentOptionIndex++;
      }
    },

    // 从显示列表中移除当前题目（不调用删除接口）
    hideCurrentQuestion() {
      if (this.displayQuestionList.length === 0) return;
      
      const currentQuestion = this.displayQuestionList[this.currentOptionIndex];
      const filterKey = currentQuestion.repeatId ? 'repeatId' : 'id';
      const filterValue = currentQuestion.repeatId || currentQuestion.id;
      
      this.displayQuestionList = this.displayQuestionList.filter(item => item[filterKey] !== filterValue);
      
      // 调整当前索引
      if (this.currentOptionIndex >= this.displayQuestionList.length) {
        this.currentOptionIndex = Math.max(0, this.displayQuestionList.length - 1);
      }
    ...
    // 按repeatId判断选项是否选中
    isOptionSelectedByRepeatId(repeatId, key) {
      return this.selectedOptionsByRepeatId[`${repeatId}_${key}`] === true;
    },

    // 按repeatId选择选项
    selectOptionByRepeatId(question, key) {
      const repeatId = question.repeatId;
      
      // 清除该题目之前的选择
      Object.keys(this.selectedOptionsByRepeatId).forEach(k => {
        if (k.startsWith(`${repeatId}_`)) {
          delete this.selectedOptionsByRepeatId[k];
        }
      });
      
      // 设置新选择
      this.selectedOptionsByRepeatId[`${repeatId}_${key}`] = true;
      
      // 判断答案是否正确
      const correctAnswer = question.answer?.toLowerCase();
      const isCorrect = correctAnswer === key.toLowerCase();
      
      this.answerResultsByRepeatId[repeatId] = isCorrect ? 'true' : 'false';
    },

    // 按repeatId获取答题结果
    getAnswerResultByRepeatId(repeatId) {
      return this.answerResultsByRepeatId[repeatId] || null;
    },

    // 按repeatId切换解析显示
    toggleExplainByRepeatId(repeatId) {
      this.showExplainByRepeatId[repeatId] = !this.showExplainByRepeatId[repeatId];
    },
    ...
    copyQuestionWithContext(questionIndex) {
      ...
      try {
        ...
        navigator.clipboard.writeText(copyContent).then(() => {
          ...
          // 添加题目索引到已复制集合中
          this.copiedQuestions.add(questionIndex);
          
          // 2秒后自动恢复为复制图标
          setTimeout(() => {
            this.copiedQuestions.delete(questionIndex);
          }, 2000);
        }).catch(err => {
          console.error('复制失败:', err);
          this.$message.error('复制失败，请手动复制');
...
@keyframes pulse {
  0% { box-shadow: 0 0 8px rgba(255, 152, 0, 0.8); }
  50% { box-shadow: 0 0 12px rgba(255, 152, 0, 1); }
  100% { box-shadow: 0 0 8px rgba(255, 152, 0, 0.8); }
}
...
.explain-text {
  ...
  border-left: 5px solid rgb(115, 199, 235);
  font-size: 25px;
}
.settings-container{
  display: flex;
