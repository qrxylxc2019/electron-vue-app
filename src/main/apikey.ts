...
请仅返回关键词，用顿号分隔。`,
  ...
  // 征稿 AI 解析 URL（提取内容和截止时间）
  solicitParse: (url: string) => ({
    system: `你是一个征稿信息提取助手。用户会给你一个网页链接，请分析该链接中的征稿信息，提取出征稿内容摘要和截止时间。

输出要求：
1. content: 征稿的主题、要求等核心内容摘要（尽量详细但不超过200字）
2. time: 征稿截止日期，格式为 YYYY-MM-DD
3. 如果无法确定截止时间，time 字段返回空字符串
4. 必须返回标准 JSON 格式，不要包含任何其他文字`,
    user: `请分析以下征稿链接，提取征稿内容和截止时间：

【链接】${url}

请直接返回JSON格式，格式如下：
{"content": "征稿内容摘要", "time": "YYYY-MM-DD"}`,
  }),
};
