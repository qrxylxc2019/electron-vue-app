...
// 读取题目文件
const filePath = path.join(__dirname, 'english_questions.txt');
...
// 查找或创建"高项英语"科目
let dirStmt = db.prepare("SELECT id FROM directories WHERE name = '高项英语'");
let dirRow = dirStmt.get();

let dirId;
if (!dirRow) {
  const insertDir = db.prepare("INSERT INTO directories (name, sort_order) VALUES (?, ?)");
  const result = insertDir.run('高项英语', 0);
  dirId = Number(result.lastInsertRowid);
  console.log('创建科目：高项英语，id:', dirId);
} else {
  dirId = dirRow.id;
  console.log('找到已有科目：高项英语，id:', dirId);
}
...
console.log(`成功插入 ${successCount} 道题目到"高项英语"科目下`);
